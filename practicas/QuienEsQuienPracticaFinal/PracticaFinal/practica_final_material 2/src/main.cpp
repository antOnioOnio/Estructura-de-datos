...
int main(int argc, char * argv[]){
	...
	bintree<Pregunta> mi_arbol = quienEsQuien.getArbol();

	cout << "=========== Arbol en crudo ===========" << endl;
	quienEsQuien.escribir_arbol_completo();
	quienEsQuien.elimina_personaje("Ernesto", mi_arbol.root());
	cout << "Profundidad promedio de las hojas del arbol: ";
	cout << quienEsQuien.profundidad_promedio_hojas() << endl; 
	cout << "======================================" << endl << endl << endl;


	quienEsQuien.eliminar_nodos_redundantes(mi_arbol, mi_arbol.root());
	
	quienEsQuien.usar_arbol(mi_arbol);
	...
	set<string> personajes_levantados  = quienEsQuien.informacion_jugada(mi_arbol.root());
	set<string>::iterator it = personajes_levantados.begin();
	
	cout << " === Personajes Restantes === " << endl;
