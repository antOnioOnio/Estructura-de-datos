...
//Busca un evento en la Cronología y devuelve su posición en el vector
  ...
  Cronologia Cronologia::eventosEntreAnios(int anio1, int anio2)
  {
    ...
    for (int a = anio1; a<=anio2 ; a++){
      i = buscarAnio(a);
      if (i != -1){
        nueva.aniadirEvento(this->event[i]);
      }
    }
    return nueva;
  }
  //------------------------------------------------------------------------------------
  // funcion eventosanio
  Fecha_Historica Cronologia::eventosAnio(int anio)
  {
    assert(anio >0 && anio <9999);
    int i = 0;
    Fecha_Historica nueva;
    i = buscarAnio(anio);
    if ( i != -1){
      nueva = this->event[i];
    }
  ...
  //------------------------------------------------------------------------------------
  // Operator+
  void Cronologia::operator+(const Cronologia &cro)
  {
    for (int i = 0; i< cro.neventos; i++){
      this->aniadirEvento(cro.event[i]);
    } 
    this->ordenar();
  }
  //------------------------------------------------------------------------------------
  // operator =
  Cronologia& Cronologia::operator=(const Cronologia& cro)
  {

    if( this!= & cro){
      delete [] event ;
      this->neventos = cro.neventos;
      this->reservados = cro.reservados;
        if (this->neventos > 0){
           this->event = new Fecha_Historica[this->reservados];

          for (int i = 0; i< cro.neventos ; i++){
            this->event[i]=cro.event[i];
          }
        }
        else event = 0;
    }
